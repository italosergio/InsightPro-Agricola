...
describe('localDB', () => {
  ...
  it('2.4 add insere item na lista', () => {
    const result = localDB.add(KEY, { id: '1', nome: 'João' })
    expect(result).toHaveLength(1)
    expect(result[0].nome).toBe('João')
  })

  it('2.5 update altera apenas o item correto', () => {
    localDB.add(KEY, { id: '1', nome: 'João' })
    localDB.add(KEY, { id: '2', nome: 'Maria' })
    const result = localDB.update(KEY, '1', { nome: 'João Atualizado' })
  ...
  it('2.6 delete remove apenas o item correto', () => {
    localDB.add(KEY, { id: '1', nome: 'João' })
    localDB.add(KEY, { id: '2', nome: 'Maria' })
    const result = localDB.delete(KEY, '1')
    expect(result).toHaveLength(1)
